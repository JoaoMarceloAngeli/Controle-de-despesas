...
import { formatarMoeda, hojeISO } from '../../../shared/utils/formatters'
import type { SaidaOperacional } from '../../../types'

interface SaidaOperacionalFormProps {
  itens: SaidaOperacional[]
  onAdicionar: (item: Omit<SaidaOperacional, 'id'>) => void
  onAtualizar: (id: string, dados: Partial<Omit<SaidaOperacional, 'id'>>) => void
...
export function SaidaOperacionalForm({
  ...
  function adicionar() {
    ...
    if (!descricao) return
    onAdicionar({ descricao, valor: valorNovo, data: dataNova })
    setDescricaoNova('')
    setValorNovo(0)
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="grid grid-cols-1 gap-3 rounded-lg border border-slate-200 bg-slate-50 p-4 sm:grid-cols-[2fr_1fr_1fr_auto]">
      ...
      <p className="text-right text-sm font-semibold text-slate-700">
        Total de saídas operacionais: {formatarMoeda(total)}
      </p>
    </div>
  )
}
